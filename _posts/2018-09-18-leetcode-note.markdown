...
给出一个只包含`+`,`-`运算符和括号的表达式，求出表达式的结果

解法：

    使用栈存储操作符和数字，根据运算符做处理


### Sliding Window Maximum ***

给出一个整数数组，和滑窗长度k，从数组开始向结尾滑窗，每次返回窗口内的最大值。要求时间复杂度为O(N)
...
给出一个整数字符串和一个目标整数，要求在字符串中添加若干个操作符`+`,`-`,`*`，使得字符串的表达式结果等于目标整数。返回所有符合要求的结果。
...
设计一个数据结果可以线性时间插入元素和返回所有数字的中值。

解法：

    使用两个优先队列，一个从大到小存储左半部分元素，一个从小到大存储右半部分。

### Burst Balloons ****

给出n个气球，每个气球有个得分，如果你打爆一个气球，你会得到这个气球周围两个气球和这个气球的得分乘积，找出你可以打爆所有气球的最高得分。

解法：

    使用DP算法
    dp[l][r]表示从l到r之间打爆气球的最高得分
    dp[l][r] = max(dp[l][r], dp[l][k-1] + l*arr[k]*r + dp[k+1][r])


### Count of Smaller Numbers After Self

给出一个整数数组，对于整数数组每一位返回数组右侧小于这个数的元素个数。

解法：

    构建二叉搜索树，从右往左依次添加元素并返回小于这个元素树中的个数。

    也可以使用线段树。

### Remove Duplicate Letters **

给出一个字符串，要求删除所有重复的元素使得每个字符只出现一次，而且修改后的字符串是所有可能字符串中字母序最小的。

解法：

    遍历字符串，统计字符串中每个字符出现的次数，再次遍历字符串，每次如果遇到一个字符比之前的字符小且之前的字符在后面有出现过，则删除之前的字符。


### Create Maximum Number ****

给出两个长度分别为n,m的整数数组，要求从两个数组中找出k个整数，满足在原数组的顺序且组合起来的数字最大

解法：

    解法，从两个数组分别取所有可能的组合，计算组合的最大值。
...
给出一个二维整数矩阵，找出最长的递增序列

解法：

    使用记忆化搜索，dp[i][j]存储在i行j列元素开始的最长递增序列


### Patching Array ****

给出一组排序后的正整数和整数n,要求在整数数组添加若干个元素可以满足1-n的所有数都可以由整数数组中几个数的和求出，返回需要添加的最少的数字。

解法：

    从左向右遍历，每次把当前元素加上，如果当前元素大于之前元素的和，说明少一个元素，添加这个元素
    或者加到最后数组小于n，则依次往后添加


### Palindrome Pairs

给出一系列不重复的单词，要求找出两个单词拼接使得拼接后的单词是回文字符串，返回所有可能的结果

解法：

    对每个单词遍历所有子串找出子串是回文且剩余部分的翻转在单词数组中出现过
 
### Russian Doll Envelopes ***

给出一组信封，每个信封有宽和高，如果一个信封的宽高都大于另一个信封，则可以包含另一个信封，求出最多有多少个信封可以被包含在一起
...
给出N个矩形，计算它们是否可以组成一个不交叉的矩形

解法：

    方法一：计算每个矩阵的和并更新整体的左上坐标和右下坐标，判断矩阵和和坐标夹的矩形是否一样
...
给出n个元素的整数元素，每个位置的数字k代表在当前位置可以往后跳跃k-1,k或者k+1步。
最开始在第0个位置，计算是否跳跃到最后一个位置
...
给出N×M的矩阵，矩阵元素代表柱子高度，求出矩阵组成的形状最多储水多少

解法：

    首先把边界元素入优先队列，每次取最小的元素BFS，如果碰到更小的元素，计算储水量，否则入队列


### Split Array Largest Sum ***

给一个吩咐整数数组和整数M，可以将数组分成M份，求出M份连续子数组最大和最小为多少
...
要求以O(1)时间复杂度实现一个可以增加、减少key次数并返回集合中次数最少和最多的一个key

解法：

    使用Hash+双向链表，链表存储次数和对应次数关联的key


### K-th Smallest in Lexicographical Order ****

找出1-n中字典序第k小的数，字典序是`[1 10 11 12 13 .. 2 20 ..]`
